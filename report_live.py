...
def gen_html(rows):
    ...
    for r in rows:
        for k in R:
            R[k] += r.get(k, 0)
        p = prov.setdefault(r["prov"], {"backlog": 0, "ontrip": 0, "fin": 0, "gtc": 0, "att": 0, "total": 0, "ltc": 0, "cod_gtb": 0, "kien": 0})
        for k in ("backlog", "ontrip", "fin", "gtc", "att", "total", "ltc", "cod_gtb", "kien"):
            p[k] += r.get(k, 0)
        amn = AM_OF.get(r["name"])
        if amn:
            a = am.setdefault(amn, {"bc": 0, "backlog": 0, "gtc": 0, "att": 0, "total": 0, "ltc": 0, "cod_gtb": 0, "kien": 0})
            a["bc"] += 1
            for k in ("backlog", "gtc", "att", "total", "ltc", "cod_gtb", "kien"):
                a[k] += r.get(k, 0)
...
body{background:radial-gradient(120% 62% at 50% -5%,rgba(34,197,94,.20),transparent 60%),radial-gradient(100% 55% at 92% 16%,rgba(16,185,129,.12),transparent 60%),#0a1410 !important;background-attachment:fixed}
.top{background:linear-gradient(180deg,#08130d 62%,rgba(8,19,13,0)) !important}
</style></head><body>"""

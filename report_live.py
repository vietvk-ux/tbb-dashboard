...
# ---------- DỰ PHÒNG: đọc ảnh chụp ngày mới nhất từ Supabase ----------
...
def load_snapshot():
    """Ảnh chụp NGÀY MỚI NHẤT từ Supabase → (agg-dict cho report_dashboard, backlog, 'YYYY-MM-DD').
    Trả None nếu thiếu cấu hình Supabase hoặc chưa có dữ liệu."""
...
def _write_fallback(err):
    """Fetch live lỗi → dựng trang từ snapshot Supabase + banner cảnh báo. True nếu ghi được."""
    snap = load_snapshot()
    if not snap:
        return False
    ...
    banner = (
        "<div style=\"background:linear-gradient(135deg,#7a1d22,#4a1216);"
        ...
        "Đang hiển thị <b>SỐ CHỐT ngày %s</b> từ kho dự phòng (Supabase). "
        "Chỉ số LTC không có trong bản dự phòng.</div>" % dm)

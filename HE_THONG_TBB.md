...
- `index.html` / `live.html` — TRANG CHÍNH, GẦN REALTIME (mỗi ~15'). **Menu 9 báo cáo = Bento grid (Mẫu 3, 24/09):** lưới 2 cột `.menu/.mtile`, mỗi ô icon chip màu + tên + phụ đề + mini-nhịp; ô "Báo cáo tổng quan" feature span 2 cột kèm %GTC vùng lớn. Màu qua `--h` (rgb) + rgba. `_menu` list trong `gen_html`. Dải 12 tile (gồm 🚛 Còn phải giao · 📊 Tiến độ chạy · 🕘 XP muộn >9h30 · 💰 COD GTB kẹt · 3 chỉ số TikTok…) + chú giải chân trang. **(23/09) Thêm %GTC TikTok theo NV/bưu cục/AM** (`_tt_cell`/`_tt_chip`, agg AM+prov cộng vngh): bảng NV bỏ cột 'Ch' → thêm cột 🛍️GTC; meta BC/AM/tỉnh có chip 🛍️ %GTC (GTC/gán); media ≤430px thu gọn bảng cho iPhone.

...
- `index.html` / `live.html` — TRANG CHÍNH, GẦN REALTIME (mỗi ~15'). **TOÀN TRANG theo Bento (Mẫu 3, 24/09):** (1) **Menu 9 báo cáo** lưới 2 cột `.menu/.mtile`, mỗi ô icon chip màu + tên + phụ đề + mini-nhịp; ô "Báo cáo tổng quan" feature span 2 cột kèm %GTC vùng lớn (`_menu` list). (2) **Dải 12 chỉ số** bo 16px, nền radial tint + giá trị màu theo từng chỉ số (`kpis` list, mỗi ô `--h` rgb). (3) **Thanh Tồn chưa gán** = ô feature bento đỏ `.cgbento` (icon chip + số lớn + caret ▾), drill AM→bưu cục→tuyến bên trong. (4) **Hero %GTC** + **cards Theo AM/Tỉnh/Bưu cục** (`.bc`/`.bc.sub`): bỏ spine trái, nền radial tint theo màu %GTC (`--h`: good/warn/bad = xanh/amber/đỏ), viền hue, bo 16px. Tất cả dùng `--h` (rgb) + rgba (không cần color-mix) → CẢ trang là 1 hệ bento thống nhất. Dải 12 tile (gồm 🚛 Còn phải giao · 📊 Tiến độ chạy · 🕘 XP muộn >9h30 · 💰 COD GTB kẹt · 3 chỉ số TikTok…) + chú giải chân trang. **(23/09) Thêm %GTC TikTok theo NV/bưu cục/AM** (`_tt_cell`/`_tt_chip`, agg AM+prov cộng vngh): bảng NV bỏ cột 'Ch' → thêm cột 🛍️GTC; meta BC/AM/tỉnh có chip 🛍️ %GTC (GTC/gán); media ≤430px thu gọn bảng cho iPhone.
